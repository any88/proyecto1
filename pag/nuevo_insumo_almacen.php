...
$objCategAlmacen=new CategoriaAlmacenController();
$msg="";
$error=0;
...
$p_id_proveedor="";
$p_nombre_proveedor="";
$listaCategoriasAlmacen=$objCategAlmacen->MostrarCategoria();
$p_id_categoriaAlmacen="";
$p_nombre_categoria_Almacen="";
$p_cantidad=0;
$p_precio=0;
$p_precio_venta=0;
$p_fecha_compra="";
...
if($_POST)
{
    ...
    if(isset($_POST['categoria'])){$p_id_categoriaAlmacen= eliminarblancos($_POST['categoria']);}
    ...
    if($p_id_categoriaAlmacen=="" || $p_id_insumo=="" || $p_id_proveedor=="" || $p_cantidad==0 || $p_cantidad=="")
    {
        $msg="<div class='alert alert-danger alert-dismissable'>"
            . "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>"
            . "Error! Los campos 'Categoria' ,'Insumo','Cantidad' y 'Proveedor' no pueden quedar vac&iacute;os.</div>";
        $error++;
    }
    else
    {
          ...
          ?>
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <div class="col-md-9">
                        <label>Seleccione la categor&iacute;a del Insumo</label>
                        <select name="categoria" class='form-control selectpicker' data-live-search='true' required="true">
                            <option value="">--SELECCIONE CATEGORIA--</option>
                            <?php 
                                for ($i = 0; $i < count($listaCategoriasAlmacen); $i++) 
                                {
                                    $id_cat_almacen=$listaCategoriasAlmacen[$i]->getId_categoria_almacen();
                                    $nombre_cat_almacen=$listaCategoriasAlmacen[$i]->getNombre_categoria();
                                    $selected="";
                                    if($id_cat_almacen==$p_id_categoriaAlmacen){$selected="selected='selected'";}
                                    echo "<option value='$id_cat_almacen' $selected>$nombre_cat_almacen</option>";
                                }
                            ?>
                        </select>
